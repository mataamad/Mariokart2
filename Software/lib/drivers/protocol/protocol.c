...
void proto_init(address_t board_address) {
    ...
    ConfigureTc(); //this is an interrupt called every 250ms checking for proto timeout or something
}
...
void int_to_data(int val, unsigned char data[]) {
    unsigned char* val_p = (unsigned char*)&val;
    data[1] = val_p[0];
    data[2] = val_p[1];
    data[3] = val_p[2];
    data[4] = val_p[3];
